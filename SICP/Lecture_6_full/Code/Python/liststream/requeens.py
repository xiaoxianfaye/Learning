...
def fillrows(size, row):
    if row == 0:
        return cons_stream(emptyboard(), the_empty_stream())

    return filter_stream(lambda poses: issafe(poses), 
                         flatmap(lambda restposes: [adjoinpos(row, col, restposes) for col in enum_interval(1, size)], 
                                 fillrows(size, row - 1)))
...
def is_not_samecol(newpos, restposes):
    return nonematch(lambda restpos: newpos[1] == restpos[1], restposes)

def is_not_diagonal(newpos, restposes):
    return nonematch(lambda restpos: abs(newpos[0] - restpos[0]) == abs(newpos[1] - restpos[1]), restposes)
